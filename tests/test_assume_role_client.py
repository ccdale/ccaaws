...
from ccaaws.session import assumeRoleClient


@patch("ccaaws.session.boto3.Session")
@patch("ccaaws.session.session")
def testAssumeRoleClientCreatesSessionWhenNoneGiven(
    ...
    mockBotoSession.return_value.client.assert_called_once_with("s3")
    assert result is mockBotoSession.return_value.client.return_value


@patch("ccaaws.session.boto3.Session")
def testAssumeRoleClientUsesGivenSession(mockBotoSession: MagicMock) -> None:
    givenSess = MagicMock()
